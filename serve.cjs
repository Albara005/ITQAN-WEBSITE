...
- **إلزامي:** ضع شعار إتقان في ترويسة كل ملخص عبر هذا الوسم بالضبط (لا تحذفه ولا تغيّر رابطه أبدًا):
  <img src="https://www.itqanoman.co/logo.png" alt="إتقان" style="height:70px;width:auto;display:block" />
  ادمجه بأناقة في تصميم الترويسة (يمكن وضعه بجانب العنوان أو فوقه). (ألوان الهوية المقترحة: كحلي #0c1a2b وذهبي #cda14a — لكنك حرّ في لوحة الألوان.)
- **حقوق إتقان (إلزامي لحماية الملخص من الانتحال):**
  1) تذييل ثابت في كل صفحة (position:fixed; bottom:0) بخط صغير أنيق نصّه: «© إتقان — جميع الحقوق محفوظة · www.itqanoman.co».
  2) علامة مائية خفيفة جدًا في خلفية الصفحة: كلمة «إتقان» مكرّرة أو مائلة بشفافية منخفضة (opacity ~0.05–0.08) لا تعيق القراءة إطلاقًا، مع -webkit-print-color-adjust:exact لتظهر عند الطباعة.
